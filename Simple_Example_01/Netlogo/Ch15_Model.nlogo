...
to setup
  ...
  create-alewives n-alewives [
    setxy (min-pxcor + random world-width)
          (min-pycor + random world-height)
    set shape "fish"
    set color cyan
    ...
    set home-patch      patch max-pxcor 0
    set migration-patch patch min-pxcor 0
...
to go
  ...
  ask alewives [
    ...
    ;; Step 1: migrate — face home-patch and step forward
    if landward-migration? and not at-destination? [
      face home-patch
      fd speed
    ]

    ;; Step 2: school — separation, cohesion, alignment, speed
    school
  ...
  ask alewives with [not at-destination?] [
    set color cyan
  ]
