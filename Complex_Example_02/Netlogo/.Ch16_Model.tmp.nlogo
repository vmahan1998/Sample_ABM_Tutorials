...
to setup
  ...
  create-alewives 20 [
    ...
    set migration-patch patch (round (max-pxcor / 2)) (min-pycor + 1)
    ...
    set migration-action       "landward"
    set migration-destination  "home"
    set migration-day          0
    ...
    set age               "adult"
    set age-num           4
    set sex               ifelse-value (random-float 1 < 0.5) ["male"] ["female"]
...
to go
  ...
  ask alewives with [start-migration? and landward-migration?] [
  ...
  ask alewives with [start-migration? and not at-destination?] [
    ifelse selective-tidal-transport?
      [ set color orange ]
      [ set color cyan   ]
  ]
